...
public class LatteScheduledThreadPoolExecutor implements ScheduledExecutorService {
    ...
    int currentCorePoolSize = 1;
    public LatteScheduledThreadPoolExecutor(String name, int corePoolSize, int maximumPoolSize, long keepAliveTime) {
        this.delegate = new ScheduledThreadPoolExecutor(
            corePoolSize,
            new ThreadFactoryBuilder().setNameFormat("latte-" + name + "-%d").build(),
            new ThreadPoolExecutor.DiscardOldestPolicy()
        );
        this.delegate.setMaximumPoolSize(maximumPoolSize);
        this.delegate.setKeepAliveTime(keepAliveTime, TimeUnit.SECONDS);
    }
